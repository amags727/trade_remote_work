...
bs_br = import_file('1) data/3_bs_br_data.parquet',char_vars =  c('firmid')) %>%
  .[year %in% year_range] %>% distinct(firmid, year, .keep_all = T)
if(dummy_version){ bs_br[,NACE_BR := first(NACE_BR), by = firmid]}
  
age_data = import_file('9_age_data/9b_firm_lvl_birth_data.parquet',
                       col_select = c('birth_year', 'first_export_year', 'firmid', 'last_observed'))  
ctry_lvl_age_data = import_file('9_age_data/9c_firm_ctry_lvl_birth_data.parquet')
