...
if(exporting) fwrite(bs_br_data, '1) data/3_bs_br_data.parquet')
...
bs_br_data =  fread('1) data/3_bs_br_data.parquet', select= c('firmid', 'year', 'deflated_dom_turnover', 'NACE_BR','empl'),
...
bs_br_data = fread('1) data/3_bs_br_data.parquet', colClasses = list(character= 'firmid'))
...
bs_br_data =  fread( '1) data/3_bs_br_data.parquet', colClasses = list(character= 'firmid'))[firmid %in% unique(customs_data_product_level[, firmid])]
