...
# set parameters ----------------------------------------------------------------------
current_import_name = '2025_06_30_import_package'
...
code_to_run = data.frame(
  file_path = c( '2) code/1_cleaning/4_process_linkedin.R',
                '2) code/1_cleaning/5_make_firm_yr_dta.R',
                '2) code/2_analysis/round_4/2_firm_yr_lvl_analysis.R'))

non_output_files_to_export = data.frame(file_path = NA, directory = NA);
rm(non_output_files_to_export)
...
if(exists('non_output_files_to_export')) write_parquet(non_output_files_to_export,paste0(import_dir,'/5_non_output_files_to_export.parquet'))
