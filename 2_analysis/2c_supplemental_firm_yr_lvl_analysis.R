...
int_entry_command = gsub('log_comp_data \\+ log_comp_data_lag1','log_comp_data*interaction + log_comp_data_lag1*interaction', rev_command)

# 2 generate variations ----------------------------------------------------------------------
interactions = con_fil(firm_yr, 'prior', 'dom_turnover_bar', 'log_age', 'age_bracket')
baseline_regs = data.frame(command = c(rev_command, entry_command))

size_quartile_regs = data.table(
  command = rep(c(rev_command, entry_command),each = 12), quartile_num = rep(1:4,6), 
  q_extension = rep(rep(c('', '_nace', '_ever_data'),each = 4),2)) %>% rowwise() %>% 
  mutate( command = gsub('firm_yr', paste0('firm_yr[log_dom_turnover_bar_quartile', q_extension, '==', quartile_num,"]"), command))
