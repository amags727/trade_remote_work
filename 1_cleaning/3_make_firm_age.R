...
write_parquet(admin_birth_data %>% numeric_firmid(.), '1) data/9_age_data/9a_admin_birth_data.parquet')
}
# generate complete birth data ------------------------------------------
#### prepare customs birth data
customs_birth_data = import_file('1) data/6_customs_firm_level.parquet', col_select = c('year', 'firmid_num','exim')) %>% unique() %>% 
...
br_birth_data = import_file('1) data/3_bs_br_data.parquet',col_select = c('firmid_num','year','for_turnover')) %>% 
...
birth_data = birth_data %>% unique() %>% merge(customs_birth_data, all = T, by = 'firmid_num') %>% merge(br_birth_data, all = T, by = 'firmid_num') %>% 
...
## export   
write_parquet(birth_data,'1) data/9_age_data/9b_firm_lvl_birth_data.parquet')
...
birth_data = import_file('1) data/9_age_data/9b_firm_lvl_birth_data.parquet', 
                         col_select = c('firmid_num','first_import_year', 'first_export_year_customs', 'birth_year'))

customs_data = c('year', 'firmid_num','exim', 'ctry_num', 'streak_id') %>%
  import_file('1) data/6_customs_firm_level.parquet',col_select = .) 
...
write_parquet(customs_birth_data,'1) data/9_age_data/9c_firm_ctry_lvl_birth_data.parquet')
rm(list= setdiff(ls(), base_env)); gc()
# Generate streak information for firms  ----------------------------------
age_data = import_file('1) data/9_age_data/9b_firm_lvl_birth_data.parquet', col_select = c('birth_year', 'firmid_num'))


streak_info_BS = import_file('1) data/3_bs_br_data.parquet', col_select = c('firmid_num', 'year', 'for_turnover')) %>%
  .[for_turnover!=0] %>% 
...
streak_info_customs = import_file('1) data/6_customs_firm_level.parquet', col_select = export_vars) %>% .[exim == 2] %>% 
...
write_parquet(output,'1) data/9_age_data/9d_firm_lvl_export_streak_info.parquet')
